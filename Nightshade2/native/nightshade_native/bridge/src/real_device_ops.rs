    ...
    async fn mount_slew_to_coordinates(&self, mount_id: &str, ra_hours: f64, dec_degrees: f64) -> DeviceResult<()> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                mount.connect().await?;
                mount.set_target_right_ascension(ra_hours).await?;
                mount.set_target_declination(dec_degrees).await?;
    ...
    async fn mount_get_coordinates(&self, mount_id: &str) -> DeviceResult<(f64, f64)> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                let mount = AlpacaTelescope::from_server(&base_url, device_num);
                mount.connect().await?;
                let ra = mount.right_ascension().await?;
                let dec = mount.declination().await?;
                mount.disconnect().await.ok();
                return Ok((ra, dec));
            }
        }
    ...
    async fn mount_is_slewing(&self, mount_id: &str) -> DeviceResult<bool> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                let mount = AlpacaTelescope::from_server(&base_url, device_num);
                mount.connect().await?;
                let slewing = mount.slewing().await?;
                mount.disconnect().await.ok();
                return Ok(slewing);
            }
        }
    ...
    async fn mount_is_parked(&self, mount_id: &str) -> DeviceResult<bool> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                let mount = AlpacaTelescope::from_server(&base_url, device_num);
                mount.connect().await?;
                let parked = mount.at_park().await?;
                mount.disconnect().await.ok();
                return Ok(parked);
            }
        }
    ...
    async fn mount_is_tracking(&self, mount_id: &str) -> DeviceResult<bool> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                let mount = AlpacaTelescope::from_server(&base_url, device_num);
                mount.connect().await?;
                let tracking = mount.tracking().await?;
                mount.disconnect().await.ok();
                return Ok(tracking);
            }
        }
    ...
    async fn mount_set_tracking(&self, mount_id: &str, enabled: bool) -> DeviceResult<()> {
        ...
        if mount_id.starts_with("alpaca:") {
            ...
            if parts.len() >= 5 {
                ...
                let mount = AlpacaTelescope::from_server(&base_url, device_num);
                mount.connect().await?;
                mount.set_tracking(enabled).await?;
                mount.disconnect().await.ok();
                return Ok(());
            }
        }
