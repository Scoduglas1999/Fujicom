...
namespace ASCOM.ScdouglasFujifilm.Camera
{
    ...
    {
        // --- SDK Wrapper code remains the same ---
    ...
    internal static class CameraHardware
    {
        ...
        // Fix for Warning CS0649: Initialize cameraName
        ...
        // Remove LibRaw handle - managed by wrapper now
        // private static IntPtr libraw_handle = IntPtr.Zero;
        ...
        private static int cameraXSize = 11648; // Default/Example GFX100S
        private static int cameraYSize = 8736;  // Default/Example GFX100S
        private static double pixelSizeX = 3.76; // Default/Example GFX100S
        private static double pixelSizeY = 3.76; // Default/Example GFX100S
        private static int maxAdu = 65535; // Assuming 16-bit RAW
        private static bool canAbortExposure = false; // SDK likely doesn't support reliable abort
        private static bool canStopExposure = false;  // SDK likely doesn't support reliable stop during exposure
        private static bool canPulseGuide = false; // No pulse guiding via SDK
        private static bool hasShutter = true; // Camera has a shutter
        private static string sensorName = "Unknown"; // Read from SDK
        ...
        // Cached capabilities
        private static List<int> supportedSensitivities = new List<int>(); // Keep empty if SDK call fails
        private static int minSensitivity = 100; // Default
        private static int maxSensitivity = 12800; // Default
        ...
        private static List<int> supportedShutterSpeeds = new List<int>(); // Keep empty if SDK call fails
        private static double minExposure = 0.0001; // Default
        private static double maxExposure = 3600.0; // Default
        private static bool bulbCapable = true; // Default to true if SDK call fails

        // Image buffer
        private static object lastImageArray = null; // Holds the int[,] array for ASCOM
            ...
            lock (exposureLock)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        // Add GetFocusMode using GetProp if the P/Invoke signature is confirmed/works
                        // int currentFocusMode = -1;
                        // getResult = FujifilmSdkWrapper.XSDK_GetProp_Int(hCamera, FujifilmSdkWrapper.API_CODE_GetFocusMode, 0, out currentFocusMode);
                        // if(getResult == FujifilmSdkWrapper.XSDK_COMPLETE) LogMessage("StartExposure", $"GetProp(FocusMode) OK: FocusMode={currentFocusMode}");
                        // else LogMessage("StartExposure", $"GetProp(FocusMode) FAILED: Result={getResult}");
                    ...
                    // --- Allocate memory for long* plShotOpt and pass pointer ---
                    ...
                    LogMessage("StartExposure", $"Triggering exposure via XSDK_Release (Mode: {FujifilmSdkWrapper.XSDK_RELEASE_SHOOT_S1OFF}, Options Ptr: {shotOptPtr})...");
                    int releaseStatus;
                    int releaseMode = FujifilmSdkWrapper.XSDK_RELEASE_SHOOT_S1OFF;
                    int releaseResult = FujifilmSdkWrapper.XSDK_Release(hCamera, releaseMode, shotOptPtr, out releaseStatus); // Pass pointer to long
                    ...
                    int bufferMillis = 2000; // Add buffer time for camera processing before checking image
        ...
        // Removed CalculateRawDataOffset - no longer needed
